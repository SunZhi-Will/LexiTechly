...
[![版本](https://img.shields.io/badge/版本-1.4.1-blue.svg)](CHANGELOG.md)
...
- 📝 智慧單字列表與 AI 詞彙分析
  ...
  - 詳細用法說明
- 🔍 **沉浸式查閱模式**
  - 網頁右上角浮動 Logo，一鍵切換查閱模式
  - 滑鼠懸停單字 3 秒自動顯示詳細資訊
  - 單字高亮選取效果，視覺化學習體驗
  - 精美的單字卡片顯示：中文翻譯、詞性、CEFR 等級、例句
- 🎯 現代化的使用者介面
  - 美觀的 Toast 通知系統
  - 自訂確認對話框
  - 流暢的載入動畫
...
### 7. 沉浸式查閱模式 🔍
- **啟用查閱模式**：
  - 在任何英文網頁上，查看右上角的浮動 LexiTechly Logo
  - 點擊 Logo 切換查閱模式（Logo 會變色表示狀態）
- **即時單字查詢**：
  - 啟用查閱模式後，將滑鼠懸停在任何英文單字上
  - 停留 3 秒後自動顯示單字詳細資訊
  - 單字會有高亮選取效果，提供視覺回饋
- **精美資訊卡片**：
  - 單字標題和 CEFR 等級標籤
...
**Q: 查閱模式無法正常運作**
- 檢查是否已設定 API Key
- 確認網頁協議為 http:// 或 https://（不支援 chrome:// 等內部頁面）
- 重新整理頁面，浮動 Logo 應該會重新出現
- 檢查瀏覽器控制台是否有錯誤訊息

**Q: 單字懸停沒有反應或高亮效果不顯示**
- 確認已點擊 Logo 開啟查閱模式（Logo 應變成藍色）
- 嘗試懸停在純文字區域的英文單字上
- 避免懸停在圖片、連結或其他特殊元素上
- 確保滑鼠停留滿 3 秒
